...
def convert_problem_to_json(problem_path):
    ...
    # Read step_by_step_solution.tex
    ...
    # Read learn_topic_beforehand.tex
    ...
    probem_data_json_path = os.path.join(problem_path, 'problem_data.json')
    if os.path.exists(probem_data_json_path):
        with open(probem_data_json_path, 'r') as file:
            probem_data_json= json.load(file)
            problem_data['test_cases'] = probem_data_json['testcases']
            problem_data['severity'] = probem_data_json['severity']
            problem_data['id'] = probem_data_json['problem_id']
            problem_data['name'] = probem_data_json['name'].title()
...
def convert_problems_to_json(base_path):
    ...
    for course_name in os.listdir(base_path):
        ...
        if os.path.isdir(course_path):
            problems = []
            for problem_name in os.listdir(course_path):
                problem_path = os.path.join(course_path, problem_name)
                if os.path.isdir(problem_path):
                    problem_data = convert_problem_to_json(problem_path)
                    problems.append(problem_data)
            # Sort problems by id
            problems.sort(key=lambda x: x['id'])
            courses[course_name] = problems
